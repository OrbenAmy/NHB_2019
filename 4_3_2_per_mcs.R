...
# R-Script 4.3.2: Combining M=mcs permutations run over the computer cluster
##########################################################################################
library(tidyverse)
library(data.table)
...
#######################################################
iterations <- 500
specifications <- 20004

for (i in 1:iterations){
assign(paste0("per_", i), read.csv(paste0("mcs_permutation_frame.1863563.", i, ".csv"), nrow=2))
}
...
results_mcs_sca <- results_mcs_sca_total

results_mcs_sca_cont <- results_mcs_sca %>% filter(controls == "Controls")
results_mcs_sca_ncont <- results_mcs_sca %>% filter(controls == "No Controls")

results_mcs_sca_sig <- results_mcs_sca %>% filter(p_value < 0.05)
results_mcs_sca_cont_sig <- results_mcs_sca %>% filter(controls == "Controls" & p_value < 0.05)
results_mcs_sca_ncont_sig <- results_mcs_sca %>% filter(controls == "No Controls" & p_value < 0.05)

### 1. effect sizes
mean.obs <- mean(results_mcs_sca$effect, na.rm = TRUE)
mean.obs.c <- mean(results_mcs_sca_cont$effect, na.rm = TRUE)
mean.obs.nc <- mean(results_mcs_sca_ncont$effect, na.rm = TRUE)
...
sign.obs <- pmax(table(sign(results_mcs_sca$effect))[[-1]],table(sign(results_mcs_sca$effect))[[1]])
sign.obs.c <- pmax(table(sign(results_mcs_sca_cont$effect))[[-1]],table(sign(results_mcs_sca_cont$effect))[[1]])
sign.obs.nc <- pmax(table(sign(results_mcs_sca_ncont$effect))[[-1]],table(sign(results_mcs_sca_ncont$effect))[[1]])
...
sign.sig.obs <- pmax(table(sign(results_mcs_sca_sig$effect))[[-1]],table(sign(results_mcs_sca_sig$effect))[[1]])
sign.sig.obs.c <- pmax(table(sign(results_mcs_sca_cont_sig$effect))[[-1]],table(sign(results_mcs_sca_cont_sig$effect))[[1]])
sign.sig.obs.nc <- pmax(table(sign(results_mcs_sca_ncont_sig$effect))[[-1]],table(sign(results_mcs_sca_ncont_sig$effect))[[1]])
