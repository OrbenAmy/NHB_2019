...
# R-Script 4.1.2: Combining YRBS permutations run over the computer cluster
##########################################################################################
library(tidyverse)
library(data.table)
...
assign(paste0("per_", i), read.csv(paste0("yrbs_permutation_frame.1863043.", i, ".csv"), nrow=2))
}
...
results_yrbs_sca_cont <- results_yrbs_sca %>% filter(controls == "Controls")
results_yrbs_sca_ncont <- results_yrbs_sca %>% filter(controls == "No Controls")

results_yrbs_sca_sig <- results_yrbs_sca %>% filter(p_value < 0.05)
results_yrbs_sca_cont_sig <- results_yrbs_sca %>% filter(controls == "Controls" & p_value < 0.05)
results_yrbs_sca_ncont_sig <- results_yrbs_sca %>% filter(controls == "No Controls" & p_value < 0.05)

### 1. effect sizes
mean.obs <- mean(results_yrbs_sca$effect, na.rm = TRUE)
mean.obs.c <- mean(results_yrbs_sca_cont$effect, na.rm = TRUE)
mean.obs.nc <- mean(results_yrbs_sca_ncont$effect, na.rm = TRUE)
...
sign.obs <- pmax(table(sign(results_yrbs_sca$effect))[[-1]],table(sign(results_yrbs_sca$effect))[[1]])
sign.obs.c <- pmax(table(sign(results_yrbs_sca_cont$effect))[[-1]],table(sign(results_yrbs_sca_cont$effect))[[1]])
sign.obs.nc <- pmax(table(sign(results_yrbs_sca_ncont$effect))[[-1]],table(sign(results_yrbs_sca_ncont$effect))[[1]])
...
## only significant negative (NEED TO CHECK IF RERUN)
#sign.sig.obs <- pmax(table(sign(results_yrbs_sca_sig$effect))[[-1]],table(sign(results_yrbs_sca_sig$effect))[[1]])
#sign.sig.obs.c <- pmax(table(sign(results_yrbs_sca_cont_sig$effect))[[-1]],table(sign(results_yrbs_sca_cont_sig$effect))[[1]])
#sign.sig.obs.nc <- pmax(table(sign(results_yrbs_sca_ncont_sig$effect))[[-1]],table(sign(results_yrbs_sca_ncont_sig$effect))[[1]])
sign.sig.obs <- table(sign(results_yrbs_sca_sig$effect))[[1]]
sign.sig.obs.c <- table(sign(results_yrbs_sca_cont_sig$effect))[[1]]
sign.sig.obs.nc <- table(sign(results_yrbs_sca_ncont_sig$effect))[[1]]
